...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query(value = "SELECT p.id, p.user_id, p.content, p.createdat, p.updatedat, p.status, p.cover, u.username, u.avatar, " +
            ...
            "FROM indidea.post p left join user u on u.id = p.user_id where p.status = 1 order by p.createdat desc limit :limit offset :offset", nativeQuery = true)
    List<Map<String, Object>> findByStatus(int user_id, int limit, int offset);

    @Query(nativeQuery = true, value = "select * from (SELECT p.id, p.user_id, p.content, p.createdat, p.updatedat, p.status, p.cover, u.username, u.avatar, " +
            ...
            "FROM indidea.post p left join user u on u.id = p.user_id where p.status = 1) a where like_check = 1 order by a.createdat desc limit :limit offset :offset")
    ...
    @Query(value = "SELECT p.id, p.user_id, p.content, p.createdat, p.updatedat, p.status, p.cover, u.username, u.avatar, " +
            ...
            "FROM indidea.post p left join user u on u.id = p.user_id where p.status = 1 and p.user_id = :userId order by p.createdat desc limit :limit offset :offset", nativeQuery = true)
